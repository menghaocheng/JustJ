 ...
 *
 *<p>Note that when the expected length of the response APDU is specified in the constructors, 
 *the actual length (Ne) must be specified, not its encoded form (Le). Similarly, 
 *{@link #getNe()} returns the actual value Ne. 
 *In other words, a value of 0 means "no data in the response APDU" rather than "maximum length." 
 *</p>
 *
 *<p>This class supports both the short and extended forms of length encoding for Ne and Nc. 
 *However, note that not all terminals and Smart Cards are capable of accepting APDUs that use 
 *the extended form. 
 *</p>
 *
 *<p>For the header bytes CLA, INS, P1, and P2 the Java type int is used to represent the 8 bit 
 *unsigned values. In the constructors, only the 8 lowest bits of the int value specified by the 
 *application are significant. The accessor methods always return the byte as an unsigned value 
 *between 0 and 255. 
 *</p>
 *
 *<p>
 *Instances of this class are immutable. Where data is passed in or out via byte arrays, defensive 
 *cloning is performed.
 *</p>
 */
public final class CommandApdu implements Parcelable{
    ...
    // max length we supported
    private static final int MAX_LC = 255;
    private static final int MAX_LE = 256;


	public static final Creator<CommandApdu> CREATOR = new Creator<CommandApdu>() {
	...
	public CommandApdu(Parcel source) {
     ...
     *
     * @throws IllegalArgumentException if data.length is greater than 255 
     */
     ...
     *   or if dataLength is greater than 255
     */
    public CommandApdu(int cla, int ins, int p1, int p2, byte[] data,
     ...
     *
     * @throws IllegalArgumentException if data.length is greater than 255 
     *   or if ne is negative or greater than 256
     */
     ...
     *    ********* We don't support extended apdu now **********
    ...
    private void parse() {
		...
		if (l1 != 0) {
			...
			throw new IllegalArgumentException("Invalid APDU: Extended Apdu command is not support now!");
		}
		/*if (apdu.length < 7) {
		    throw new IllegalArgumentException
			("Invalid APDU: length=" + apdu.length + ", b1=" + l1);
    ...
    /**
     * Returns the bytes of APDU which can transmit to the raw local function.
     *
     * @return the bytes to transmit.
     */
    public byte[] getTransmitBytes(){
    	/*
    	byte[] transmitBytes = new byte[8 + nc];
    	System.arraycopy(apdu, 0, transmitBytes, 0, 4); // copy the head
    	transmitBytes[4] = (byte)(nc >> 8);
    	transmitBytes[5] = (byte)nc;
    	System.arraycopy(transmitBytes, 0, apdu, 0, 4);
    	System.arraycopy(apdu, dataOffset, transmitBytes, 6, nc);
    	transmitBytes[6+nc] = (byte)(ne >> 8);
    	transmitBytes[7+nc] = (byte)ne;
